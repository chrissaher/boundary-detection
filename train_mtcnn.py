...
import math
from model import *
from parse_images import get_training_data_mtcnn
...
def get_next_batch(gt_images, gt_labels, gt_boxes, iter, batch_size, shuffle=True):
    ...
    for gt_image in gt_images[start_point:end_point]:
        ...
        img = cv2.resize(img, (160, 30)) # To keep aspect ratio
...
def train(epochs = 5000, batch_size=16, lr=0.0001, use_cuda=True):
    print('Epochs: ', epochs)
    ...
    gt_images, gt_labels, gt_boxes = get_training_data_mtcnn(max_w = 1280, max_h = 240)
    ...
    for epoch in range(epochs):
        ...
        for iter in tqdm(range(n_iterations)):
            ...
            cls_pred_l, cls_pred_r, box_pred = net(im_tensor)
        ...
        if epoch % 50 == 0:
            print("SAVING MODEL")
            torch.save(net, './models/640x120_mtcnn/model_epoch_{}.ckpt'.format(epoch))
