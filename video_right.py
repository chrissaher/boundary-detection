...
def main():
    ...
    for filename in files:
        ...
        output_path = os.path.join('output', filename.split('.')[0] + "_right.mp4")
        ...
        net = torch.load('./models/320x60_right/model_epoch_1500.ckpt')
        ...
        for i in tqdm(range(nb_frames)):
            ...
            image = cv2.resize(image, (320, 60)).transpose(2,0,1)
            image = np.array([image])

            im_tensor = Variable(torch.from_numpy(image).float()).cuda()
            cls_pred, box_pred = net(im_tensor)

            clsr = cls_pred[0].data.cpu().numpy()
            mr,br = box_pred[0].data.cpu().tolist()
